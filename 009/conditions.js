...
if (
  (digitA > digitB && digitA < digitC) ||
  (digitA < digitB && digitA > digitC)
) {
  console.log(`Middle vlaue is A = ${digitA}`);
} else if (
  (digitB > digitA && digitB < digitC) ||
  (digitB < digitA && digitB > digitC)
) {
...
if (candles < 1000) {
  ...
  console.log(
    `Purchasing ${candles} candles costs ${(candles * 0.97).toFixed(2)} EUR`
  );
} else if (candles >= 2000) {
  console.log(
    `Purchasing ${candles} candles costs ${(candles * 0.96).toFixed(2)} EUR`
  );
}
...
console.log(
  `numberOne = ${numberOne}; numberTwo = ${numberTwo}; numberThree = ${numberThree}`
);
...
console.log(
  `Average without values less than 10 and greater than 90 is ${(
    sum / arr.length
  ).toFixed(3)}`
);
